...
CUPTIToolkit::CUPTIToolkit(KernelProfilingMode &mode) {
  ...
  if(cupti_config_.profiling_mode==KernelProfilingMode::cupti_onepass){
    for(uint32_t idx=0; idx< CUPTIMetricsOnePass::CUPTI_METRICS_ONE_PASS_TOTAL; idx++ )
      cupti_config_.metric_list.push_back(MetricListOnePass[idx]);
  }
  else if(cupti_config_.profiling_mode==KernelProfilingMode::cupti_detailed){
    for(uint32_t idx=0; idx< CUPTIMetricsOnePass::CUPTI_METRICS_ONE_PASS_TOTAL; idx++ )
      cupti_config_.metric_list.push_back(MetricListOnePass[idx]);
    for(uint32_t idx=0; idx< CUPTIMetricsDetailed::CUPTI_METRICS_DETAILED_TOTAL; idx++ )
      cupti_config_.metric_list.push_back(MetricListDetailed[idx]);
  }
  TI_TRACE("metrics num = {}", cupti_config_.metric_list.size());
...
bool CUPTIToolkit::init_cupti(){
  ...
  cupti_data_.chip_name = getChipNameParams.pChipName;
  TI_TRACE("cuptiDeviceGetChipName : {}",cupti_data_.chip_name);
  ...
  getCounterAvailabilityParams.ctx = CUDAContext::get_instance().get_context();
  ...
  cupti_data_.counter_availability_image.clear();
  cupti_data_.counter_availability_image.resize(getCounterAvailabilityParams.counterAvailabilityImageSize);
  getCounterAvailabilityParams.pCounterAvailabilityImage = cupti_data_.counter_availability_image.data();
  CUPTI_API_CALL(cuptiProfilerGetCounterAvailability(&getCounterAvailabilityParams));
  ...
  bool state = 0;
  state = GetConfigImage(cupti_data_.chip_name, cupti_config_.metric_list, cupti_data_.config_image, cupti_data_.counter_availability_image.data());
  if(!state){ TI_ERROR("Failed to create configImage"); }
  TI_TRACE("GetConfigImage");
  
  state = GetCounterDataPrefixImage(cupti_data_.chip_name, cupti_config_.metric_list, cupti_data_.counter_data_image_prefix);
  if(!state){ TI_ERROR("Failed to create counterDataImagePrefix"); }
  TI_TRACE("GetCounterDataPrefixImage");

  state = CreateCounterDataImage(cupti_config_.num_ranges, cupti_data_.counter_data_image, cupti_data_.counter_data_scratch_buffer, cupti_data_.counter_data_image_prefix);
...
bool CUPTIToolkit::begin_profiling(){
  ...
  // TI_TRACE("configImage.size() = {}",cupti_data_.config_image.size());
  // TI_TRACE("counterDataImage.size() = {}",cupti_data_.counter_data_image.size());
  beginSessionParams.ctx = NULL;
  beginSessionParams.counterDataImageSize = cupti_data_.counter_data_image.size();
  beginSessionParams.pCounterDataImage = &(cupti_data_.counter_data_image[0]);
  beginSessionParams.counterDataScratchBufferSize = cupti_data_.counter_data_scratch_buffer.size();
  beginSessionParams.pCounterDataScratchBuffer = &(cupti_data_.counter_data_scratch_buffer[0]);
  ...
  setConfigParams.pConfig = &(cupti_data_.config_image[0]);
  setConfigParams.configSize = cupti_data_.config_image.size();
...
bool CUPTIToolkit::calculate_metric_values() {
  ...
  if (!cupti_data_.counter_data_image.size()) {
    TI_WARN("Counter Data Image is empty!");
    return false;
  }

  NVPW_CUDA_MetricsContext_Create_Params metricsContextCreateParams = { NVPW_CUDA_MetricsContext_Create_Params_STRUCT_SIZE };
  metricsContextCreateParams.pChipName = cupti_data_.chip_name.c_str();
  RETURN_IF_NVPW_ERROR(false, NVPW_CUDA_MetricsContext_Create(&metricsContextCreateParams));
  ...
  getNumRangesParams.pCounterDataImage = &(cupti_data_.counter_data_image[0]);
  ...
  for (size_t rangeIndex = 0; rangeIndex < getNumRangesParams.numRanges; ++rangeIndex) {
    ...
    getRangeDescParams.pCounterDataImage = &(cupti_data_.counter_data_image[0]);
    ...
    setCounterDataParams.pCounterDataImage = &(cupti_data_.counter_data_image[0]);
    ...
    // for (size_t metricIndex = 0; metricIndex < cupti_config_.metric_list.size(); ++metricIndex) {
    //   traced_records_[rangeIndex].metric_values.push_back(gpuValues[metricIndex]);
    // }
    traced_records_[rangeIndex].kernel_elapsed_time_in_ms = gpuValues[CUPTI_METRIC_KERNEL_ELAPSED_CLK_NUMS]
                                                            / gpuValues[CUPTI_METRIC_CORE_FREQUENCY_HZS]
                                                            * 1000 ; //from s to ms
    traced_records_[rangeIndex].kernel_gloabl_load_byets  = gpuValues[CUPTI_METRIC_GLOBAL_LOAD_BYTES];
    traced_records_[rangeIndex].kernel_gloabl_store_byets = gpuValues[CUPTI_METRIC_GLOBAL_STORE_BYTES];

    // uint32_t detailed_metrics_offset = CUPTI_METRICS_ONE_PASS_TOTAL;
    if(cupti_config_.profiling_mode == KernelProfilingMode::cupti_detailed){
      traced_records_[rangeIndex].utilization_ratio_sm = gpuValues[CUPTI_METRICS_ONE_PASS_TOTAL + CUPTI_METRIC_UTILIZATION_RATIO_SM];
      traced_records_[rangeIndex].utilization_ratio_mem = gpuValues[CUPTI_METRICS_ONE_PASS_TOTAL + CUPTI_METRIC_UTILIZATION_RATIO_MEM];
    }
